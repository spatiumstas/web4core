...
function buildMihomoYaml(proxies, groups, providers, rules, listeners, opts) {
    ...
    if (tunOpt) {
        ...
        if (mode === 'listeners') {
            ...
            const isProviderMode = providerKeys.length > 0;
            if (isProviderMode) {
                if (providerKeys.length > 1) {
                    for (let i = 0; i < providerKeys.length; i++) {
                        const pn = providerKeys[i];
                        if (!pn) continue;
                        tunListeners.push(buildTunListener(i, `SUB-${pn}`));
                    }
                } else {
                ...
                if (proxyList.length > 1) {
                    for (let i = 0; i < proxyList.length; i++) {
                        const name = proxyList[i]?.name;
                        if (name) tunListeners.push(buildTunListener(i + 1, name));
                    }
                }
            }
